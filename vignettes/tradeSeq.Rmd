...
Here we fit the `tradeSeq` negative binomial generalized additive model (NB-GAM). Please see the [fitGAM vignette](https://bioconductor.org/packages/release/bioc/vignettes/tradeSeq/inst/doc/fitGAM.html) for an extensive description on how to use this function, tune its options and modify its output.

We first need to decide on how to choose the number of knots. This is done with the __evaluateK__ function. This takes a little time to run so it is not run hr

```{r, eval=FALSE}
set.seed(5)
icMat <- evaluateK(counts = counts, sds = crv, k = 3:20, 
...
For more explanations on the outputs from __evaluateK__, we once again refer users to the [fitGAM vignette](https://bioconductor.org/packages/release/bioc/vignettes/tradeSeq/inst/doc/fitGAM.html). Here, we pick `nknots = 6`.

We then run the __fitGAM__ function.
