...
In this vignette, we use `tradeSeq` downstream of `slinghsot`[@Street2018a] but it can be used downstream of any trajectory. In particular, if you want to use `tradeSeq` downstream of `monocle`[@Qiu2017] and `monocle3`[@Cao2019], please refer to our [Monocle vignette](https://statomics.github.io/tradeSeq/articles/Monocle.html).
...
compare <- dplyr::inner_join(
  patternRes %>% 
    dplyr::mutate(Gene = rownames(patternRes),
                  pattern = waldStat) %>%
    dplyr::select(Gene, pattern),
  endRes %>% 
    dplyr::mutate(Gene = rownames(endRes),
                  end = waldStat) %>%
    dplyr::select(Gene, end),
  by = c("Gene" = "Gene")) %>%
  dplyr::mutate(transientScore = (min_rank(desc(end)))^2 + (dense_rank(pattern))^2)
...
head(
  compare %>% 
    dplyr::arrange(desc(transientScore)) %>% 
    dplyr::select(Gene),
  n = 5
)
```
