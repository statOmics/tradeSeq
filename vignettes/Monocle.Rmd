...
cds <- orderCells(cds)
cds <- ordeCells(cds, root_state = 5)
```
...
```{r}
gamModels <- fitGAM(cds, verbose = TRUE)
```

Then, you can follow along the other vignette

# Monocle3

We have no plan yet to include a S4 method for monocle3 while it is not on CRAN or Bioconductor and the format is still moving. However, here is how to do it as of version '0.2', for a fully connected graph. You can install Monocle3 from [here](https://cole-trapnell-lab.github.io/monocle3/docs/installation/)
