...
plot(rd, col = brewer.pal(9, "Set1")[celltype], main = "color by cell type", xlab = "UMAP1", ylab = "UMAP2", pch = 16, cex = 2 / 3)
lines(crv, lwd = 2)
...
filt <- rowSums(counts > 5) > ncol(counts)/100
counts <- counts[filt, ]
...
```{r}
cols <- colorRampPalette(c("yellow","red"))(16)
g <- Hmisc::cut2(log(assays(se)$counts[sigGeneStart,] + 1))
plot(rd, col = cols[g],
     main = paste0("color by expression of ", sigGeneStart),
     xlab = "UMAP1", ylab = "UMAP2", pch = 16, cex = 2 / 3)
lines(crv, lwd = 2)
```
...
```{r}
cols <- colorRampPalette(c("yellow","red"))(13)
...
```{r}
earlyGenes <- earlyDrivers(gamList)

plot(lapply(gamList, function(x) x$coefficient[1] / x$coefficient[11]) %>% unlist, -log10(earlyGenes$coeff1))

ggplot(earlyGenes %>% gather(key = "coefficient", value = "pval"),
       aes(x = factor(coefficient, levels = paste0("coeff", 1:10)),
                      y = -log10(pval))) +
  geom_boxplot() +
  geom_hline(yintercept = -log10(0.05), col = "red") +
  labs(x = "coefficients", y = "-log10 of p-values") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.6))

earlyGenes %>% mutate(gene = rownames(earlyGenes)) %>%
               select(gene, coeff1) %>% 
               top_n(-6, coeff1) %>%
               arrange(coeff1)
...
cols <- colorRampPalette(c("yellow", "red"))(10)
g <- Hmisc::cut2(log(assays(se)$counts["Slc25a5", ] + 1))
plot(rd, col = cols[g], main = paste0("color by expression of Slc25a5"),
  xlab = "UMAP1", ylab = "UMAP2", pch = 16, cex = 2 / 3)
lines(crv, lwd = 2)

plotSmoothers(gamList[["Slc25a5"]])
```
# References
