...
```{r}
plotSmoothers(gamList[["Aqp1"]])
```

Or, we can visualize its expression in UMAP space.

```{r}
cols <- colorRampPalette(c("yellow", "red"))(8)
g <- Hmisc::cut2(log(assays(se)$counts["Aqp1", ] + 1))
plot(rd, col = cols[g], main = paste0("color by expression of Aqp1"),
  xlab = "UMAP1", ylab = "UMAP2", pch = 16, cex = 2 / 3)
lines(crv, lwd = 2)
```
# References
