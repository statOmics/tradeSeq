...
sigGeneStart <- names(gamList)[oStart[2]]
...
```{r}
plotGeneCount(rd, crv, counts, gene = sigGenePat)
```
...
```{r}
sigGeneEarly <- names(gamList)[earlyGenes[,2] == min(earlyGenes[,2])]
head(sigGeneEarly)
...
```{r}
plotGeneCount(rd, crv, counts, gene = "Aqp1")
```
