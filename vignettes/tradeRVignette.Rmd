...
The function `earlyDETest` implements a statistical method to tests the null hypothesis of whether for each knot, the coefficents are equal across lineages. 

```{r}
earlyGenes <- earlyDETest(gamList, output = "wald")
```

Similar to above, we can plot the estimated functions for the most significant gene, where there is a difference in the second coefficient, since we have seen above that the second knot is placed right after the branching.

```{r}
oPat <- order(earlyGenes[,"knot2"], decreasing = TRUE)
